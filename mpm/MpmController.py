...
from mpm.models import ActiveMotors, organization_lot_ids
...
class MpmController:
    ...
    def AddActiveMotor(request):
        ...
        if id == 0 or id == '0':
            ...
            motorModel.formulation_tailoring_liner_propellant_date = request[
            ...
            if current_year != '':
                if current_sys == '' and current_comp == '':
                    ...
                    totCurrentYearObservation = ActiveMotors.objects.filter(year__year=current_year,
                                                                            ...
                                                                            component_type=current_comp).count()
                    totCurrentYearOverdue = ActiveMotors.objects.filter(year__year=current_year,
                                                                        overall_status="Overdue", system=current_sys,
                                                                        component_type=current_comp).count()
            else:
